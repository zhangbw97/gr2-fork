...
class ParticleEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None, safety_cost_callback=None,
        ...
        self.action_spaces = MABox(lows=[0] * self.agent_num, highs=[1] * self.agent_num,
                                   shapes=[(world.dim_p * 2 + 1,)] * self.agent_num)
        self.observation_spaces = MABox(lows=[-np.inf] * self.agent_num, highs=[+np.inf] * self.agent_num,
                                        shapes=obs_shapes)
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            # print(action_n[i], agent, self.action_space[i])
